...
def test_not_available_settings_page_not_auth(client):
    res = client.get(reverse('account_settings'))
    assert res.status_code == 302


def test_available_settings_in_auth_client(authenticated_client):
    res = authenticated_client.get(reverse('account_settings'))
    assert res.status_code == 200


def test_correct_template_for_settings(authenticated_client):
    res = authenticated_client.get(reverse('account_settings'))
