...
class TestQuestionAndAnswer:
    ...
    def test_set_answer_as_correct(self, browser, auth_session, answers, question_page):
        with pytest.raises(NoSuchElementException):
            browser.find_element_by_class_name("correct-answer")
        browser.find_element_by_id(f"toggle-correct-answer-{answers[0].pk}").click()
